...
export function normalizeObservedPath(filePath, directory) {
...
export function commandFailed(output) {
...
export function mergeProjectBucket(dst, src) {
    const a = dst || {};
    const b = src || {};
...
export function _pruneOldSessions(state) {
    if (!state?.sessions)
        return;
...
export function _computeSessionMetrics(state, sid) {
