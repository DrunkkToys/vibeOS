...
export function postTrinity(action:string,slot?:string,level?:string,token?:string):Promise<{ok:boolean;result?:unknown;error?:string}>{const b:Record<string,string>={action};if(slot)b.slot=slot;if(level)b.level=level;if(token)b.token=token;return withPreview(()=>f("/trinity",{method:"POST",body:JSON.stringify(b)}),()=>simulatePreviewTrinity(action,slot))}
