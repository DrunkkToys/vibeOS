...
async function _appendFooter(input, output, directory) {
  ...
  try {
    ...
    function _extractText(obj) {
      ...
      if (Array.isArray(obj?.content)) return obj.content.filter(p => p?.type === "text").map(p => p.text).filter(Boolean).join("\n")
      if (Array.isArray(obj?.parts)) return obj.parts.filter(p => p?.type === "text").map(p => p.text).filter(Boolean).join("\n")
      return ""
    }
    ...
    function _setFooter(obj, text) {
      if (typeof obj?.text === "string") obj.text = text
      else if (typeof obj?.result === "string") obj.result = text
      else if (typeof obj?.content === "string") obj.content = text
      else if (Array.isArray(obj?.content)) {
        const textParts = obj.content.filter(p => p?.type === "text")
        if (textParts.length > 0) textParts[textParts.length - 1].text = text
        else obj.content.push({ type: "text", text })
      } else if (Array.isArray(obj?.parts)) {
        const textParts = obj.parts.filter(p => p?.type === "text")
        if (textParts.length > 0) textParts[textParts.length - 1].text = text
        else obj.parts.push({ type: "text", text })
      } else obj.text = text
    }
