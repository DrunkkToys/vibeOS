...
import {
  ...
  getBlackboxTracker,
  loadBlackboxState as loadBlackboxStateFromCtx,
  saveBlackboxState as saveBlackboxStateToCtx,
  extractLastUserText,
  isLikelyOffTopic,
...
let _prevOutputText = "";
const briefedProjects = /* @__PURE__ */ new Set();
const correctionSeenKeys = /* @__PURE__ */ new Set();
...
const onMessagesTransform = async (_input, output) => {
  ...
  try {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      for (const part of parts) {
        ...
        const hash = createHash("sha256").update(`tool_result
${raw}
...
const onSystemTransform = async (_input, output) => {
  if (!loadSelection().enabled) return;
  try {
    if (!latestUserIntent) {
    ...
    if (_latestBlackboxState) {
      ...
      _controlVector = await apiComputeControlVector(_latestBlackboxState, void 0, loadOptimizationMode());
    ...
    if (_controlVector && _controlVector.directives.length > 0) {
      ...
      try {
        ...
        if (res.is_looping && res.loop_intervention_level && res.loop_intervention_level !== "none") {
          ...
          const loopDirective = `[loop prevention: ${severity}] ${_latestBlackboxLoopMsg || "The conversation may be looping \u2014 try a different approach."} (level: ${res.loop_intervention_level})`;
