...
import { checkFlowRules, getFlowWarns, setFlowStateWriter, ensureProjectDocs } from "./vibeOS-lib/flow-enforcer.js";
...
import { computeDifficulty, createPatternGraph, addRouteEdge, predictBestModel, hashQuery, deserializeGraph } from "./vibeOS-lib/ml-router.js";
...
function shouldLogWarn(key, windowMs = WARN_DEDUPE_WINDOW_MS) {
    ...
    if (warnLogThrottle.size > 2000) {
        ...
        if (warnLogThrottle.size > 2000) {
            ...
            for (let i = 0; i < entries.length - 2000; i++)
...
function scanRecentScratchpad(baseDir, toolName, windowMs) {
    try {
        ...
        if (now - _lastScan < 5000)
            return null;
        _lastScan = now;
        const entries = readdirSync(baseDir);
        for (const entry of entries) {
            if (!entry.endsWith(".txt") || entry.endsWith(".summary.txt"))
                continue;
...
function isLikelyOffTopic(userText, job) {
    ...
    if (!Number.isFinite(updatedAt) || now - updatedAt > 2 * 60 * 60 * 1000)
        return false;
    ...
    try {
        ...
        try {
            ...
            if (targetModel === TRINITY_CHEAP)
                localRow.cheap += 1;
            else if (targetModel === TRINITY_MEDIUM)
                localRow.medium += 1;
            else
                localRow.high += 1;
...
function projectStructuredFromText(raw) {
    ...
    if (m1)
        brain_pct = Number(m1[1]);
    if (m2)
        worker_pct = Number(m2[1]);
...
function _pruneScratchpadDir(targetDir, opts = {}) {
    ...
    if (dataFiles > maxFiles || totalBytes > maxBytes) {
        const candidates = entries
            ...
            .map(e => {
            try {
                return { name: e, mtime: statSync(join(targetDir, e)).mtimeMs };
            }
            catch {
                return null;
            }
    ...
    try {
        ...
        for (const sid of readdirSync(SCRATCHPAD_SESSIONS_DIR)) {
            ...
            if (age > SCRATCHPAD_SESSION_TTL_MS) {
                try {
                    rmSync(sesRoot, { recursive: true, force: true });
                }
                catch { }
            }
        }
...
function observeUserCorrection(text) {
    ...
    if (key)
        recordFrictionPattern(key, summary);
}
    ...
    try {
        ...
        const collect = (rows, label) => {
            for (const row of Object.values(rows || {})) {
                ...
                if (sessions.size >= 3)
                    out.push({ label, summary: row.summary, sessions: sessions.size, lastSeen: row.lastSeen || "" });
            }
        };
...
function buildProjectBriefing(dir) {
    try {
        ...
        if (p.researchChains)
            lines.push(`  • ${p.researchChains} research domain chains found`);
        if (p.context7Bypasses)
            lines.push(`  • ${p.context7Bypasses} context7-bypass warnings`);
...
export async function DelegationEnforcer({ client, directory } = {}) {
    ...
    if (currentModel || !existsSync(TIERS_FILE)) {
        try {
            ...
            if (_medium.id === _brain.id)
                _medium = { ..._brain };
            if (_cheap.id === _brain.id || _cheap.id === _medium.id)
                _cheap = { ..._brain };
        ...
        tool: {
            trinity: tool({
                ...
                async execute({ action, slot, level } = {}) {
                    ...
                    if (typeof _lazyRefresh === "function")
                        _lazyRefresh();
                    if (!action)
                        action = "status";
                    ...
                    if (action === "rebuild") {
                        ...
                        for (const id of candidates) {
                            ...
                            if (ok)
                                probed.brain = models.find(m => m.id === id) || { id, cost: _modelCost(id), tier: _modelTier(id) };
                            else
                                failed.push("brain: " + id);
                        }
                        ...
                        for (const id of candidates) {
                            ...
                            const ok = await probeModel(id, auth);
                            if (ok)
                                probed.medium = models.find(m => m.id === id) || { id, cost: _modelCost(id), tier: _modelTier(id) };
                            else if (!failed.some(f => f.endsWith(id)))
                                failed.push("medium: " + id);
                        }
                        if (!probed.brain) {
