...
function scanClaimsInOutput(output) {
    ...
    if (typeof output === "string") {
        ...
        if (typeof payload.text === "string")
            ...
            text = payload.content.filter(p => p?.type === "text").map(p => p.text).filter(Boolean).join("\n");
        else if (Array.isArray(payload.parts))
            text = payload.parts.filter(p => p?.type === "text").map(p => p.text).filter(Boolean).join("\n");
    }
    if (!text)
        return;
    try {
...
function _collectFreeSeedModels(models) {
    ...
    for (const model of list) {
        ...
        if (!id || provider !== "opencode")
            continue;
        if (!/-free$/i.test(_normalizeSeedModelId(id)))
            continue;
        if (seen.has(id))
            continue;
        seen.add(id);
...
function scheduleMcpServerRestart() {
    ...
    if (typeof _mcpServerRestartTimer.unref === "function")
        _mcpServerRestartTimer.unref();
}
                ...
                _mcpServerRuntime = createMcpServer({
                    getState: () => ({
                        ...buildStatusPayload({
                            ...
                            tiers: (() => { try {
                                return safeJsonParse(readFileSync(getTiersFile(), "utf-8"))?.trinity;
                            }
                            catch {
                                return null;
                            } })(),
        ...
        "experimental.text.complete": async (_input, output) => {
            ...
            try {
                ...
                if (existsSync(claimFile) && statSync(claimFile).size > 0) {
                    ...
                    const cascadeRuns = cascadeLines.filter(Boolean).map(l => { try {
                        return JSON.parse(l);
                    }
                    ...
                    for (const cl of claimLines) {
                        ...
                        if (!CLAIM_PATTERNS.some(function (p) { return p.test(claimTexts); }))
                            continue;
