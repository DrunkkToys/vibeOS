...
function _loadPricingOverrides() {
  ...
  try {
    if (!existsSync6(TIERS_FILE2))
      return {};
    const st = statSync5(TIERS_FILE2);
    if (st.size > 10485760) {
      _handleStateCorruption3(TIERS_FILE2);
      _pricingOverridesCache = {};
      return {};
    }
    const raw = safeJsonParse3(readFileSync5(TIERS_FILE2, "utf-8"));
...
async function DelegationEnforcer({ client: client2, directory: directory3 } = {}) {
  ...
  if (_compatBootstrap || currentModel && String(currentModel).trim().length > 0 || existsSync16(getTiersFile())) {
    try {
      ...
      if (existsSync16(getTiersFile())) {
        ...
        _tiersData2 = {
          selection: _compatBootstrap ? { enabled: true, active_slot: currentModel ? "brain" : "medium", delegation_enforce: false, flow_enabled: false, flow_enforce: false, tdd_enforce: false, tdd_strict: false, tdd_quality: false, thinking_level: "off", onboarding_mode: "assist" } : { enabled: true, active_slot: _defaultSlots[0] || "brain", delegation_enforce: true, tdd_strict: true, tdd_quality: true, flow_enabled: false, flow_enforce: false, tdd_enforce: false, thinking_level: "off" },
          trinity: {}
        };
      }
  ...
  try {
    if (directory3 && existsSync16(directory3)) {
