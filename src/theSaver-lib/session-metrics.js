...
export function aggregateWarns(warns, filterFn) {
...
export function getSessionCost(state, sessionId) {
  const cost = state?.sessions?.[sessionId]?.cost_usd
  return cost != null ? Number(cost) : 0
}

export function computeSessionMetrics(state, sessionId) {
  ...
  const sesEdit = aggregateWarns(warns, w => w.reason?.includes("direct edit"))
  const sesCredit = aggregateWarns(warns, w => w.reason?.includes("credit"))
  const sesC7 = aggregateWarns(warns, w => w.reason?.includes("context7"))
  const sesQuota = aggregateWarns(warns, w => w.reason?.includes("quota"))
