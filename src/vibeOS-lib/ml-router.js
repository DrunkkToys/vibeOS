...
export function computeDifficulty(prompt) {
...
export function cascadeDecide(prompt, cheapModelCost, mediumModelCost, brainModelCost, cheapSuccessRate) {
...
export function createPatternGraph() {
    return {
        nodes: {},
        tiers: { cheap: [], medium: [], brain: [] },
    };
}
export function ensureNode(graph, id, kind) {
    graph.nodes[id] ??= { id, kind, count: 0, lastSeen: "", edges: {} };
    return graph.nodes[id];
}
export function addRouteEdge(graph, queryWord, modelName, tier, success) {
    ...
    const normalizedTier = tier === "budget" || tier === "low" ? "cheap"
        : tier === "mid" ? "medium"
            : tier === "high" ? "brain"
                : tier;
...
export function predictBestModel(graph, firstWord, tierPreference) {
...
export function hashQuery(prompt) {
