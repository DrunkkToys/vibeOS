...
const REGIME_ADJUSTMENTS: Record<string, number> = {
  CONVERGING: -0.10,
  CLOSED: -0.10,
  DIVERGENT: +0.15,
  REFINING: 0.00,
...
const SAFE_ACTION_MAP: Record<string, string> = {
  observe: "defer",
  explore: "act",
  defer: "defer",
  act: "act",
...
const ACTION_DECISION_BLOCKS: Record<string, { do_today: string[]; avoid_today: string[]; if_unsure: string }> = {
  act: {
    do_today: [
...
const HUMAN_ACTION_DESCRIPTIONS: Record<string, string> = {
...
export function computeModality(riskLevel: string, subRegime: string): string {
...
function riskGuidance(riskLevel: string, subRegime: string): string {
  let base: string
  if (riskLevel === "low") base = "Proceed normally"
  else if (riskLevel === "medium") base = "Proceed with caution"
  else base = "Avoid major decisions today"
...
export function humanReadableAction(action: string, continuityState: string | null | undefined): string {
...
export function buildDecisionBlock(
  action: string,
  modality: string = "suggestive",
  originalAction: string | null | undefined = null,
): DecisionBlock {
...
export function buildAdvice(
  ...
  resolutionState: ResolutionState | null | undefined = null,
  originalAction: string | null | undefined = null,
): AdviceOutput {
...
export function compressMetrics(
  diagnostics: Diagnostics,
  resolutionState: ResolutionState | null | undefined = null,
  action: string = "explore",
): { risk_level: string; guidance: string; situation_description: string } {
...
export function compressUncertainty(uncertainty: number): string {
...
export function compressEntropy(entropy: number, maxEntropy: number = 2.58): string {
  if (entropy <= 0.5) return "Your mind seems largely settled on this"
  if (entropy <= 1.2) return "Some competing thoughts, but a preference is emerging"
  if (entropy <= 1.8) return "You're genuinely torn — multiple paths feel valid"
  return "High internal conflict — you're pulled in many directions at once"
}

// ── Closure Policy ─────────────────────────────────────────────────────────

function effectiveThreshold(subRegime: string, calibrationBias: number = 0.0): number {
...
export function enforceClosure(
  action: string,
  confidence: number,
  resolutionState: ResolutionState | null | undefined = null,
  calibrationBias: number = 0.0,
): [string, boolean] {
...
export function stabilityScore(
  resolutionState: ResolutionState | null | undefined = null,
  diagnostics: Diagnostics | null | undefined = null,
): number {
  const rs = resolutionState || {}
...
export function shouldUseFastPath(
  ...
  if (!["CONVERGING", "CLOSED"].includes(regime)) return false

  const entropy = diag.entropy ?? 1.0
  if (entropy >= 0.6) return false

  if (stabilityScore(rs, diag) <= STABILITY_THRESHOLD) return false

  const contradiction = signals.feature_contradiction ?? 0.0
  if (contradiction >= 0.3) return false
...
export function buildCautionNote(verificationRequired: boolean, subRegime: string): string {
...
export function scoreUsefulness(output: Record<string, any>): UsefulnessScore {
...
export function getFallbackPlan(action: string): string[] {
  return FALLBACK_PLANS[action] || FALLBACK_PLANS.explore
}

export function getActionSuggestion(action: string): string {
  return ACTION_SUGGESTIONS[action] || ACTION_SUGGESTIONS.explore
}

export function getCuriosityPrompt(action: string): string {
