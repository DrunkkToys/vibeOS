# VibeTheOG Guard Plugin

This plugin adds a companion layer for `VibeTheOG` with:
...
```bash
VIBETHEOG_GUARD_FULL=1 bash plugins/vibetheog-guard/scripts/run-guard.sh
```

Run pre-commit hook manually:

```bash
bash plugins/vibetheog-guard/hooks/pre-commit.sh
```

## Wiring Suggestions

You can wire these scripts to your preferred runner (git hooks, task runner, or your own automations).

Example git hook setup:

```bash
ln -sf ../../plugins/vibetheog-guard/hooks/pre-commit.sh .git/hooks/pre-commit
