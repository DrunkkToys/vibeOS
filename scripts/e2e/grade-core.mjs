...
// Shared grading machinery for every scenario. The hidden tests are copied into the
// project only AFTER the session has ended, so the model can never read, run, or
// edit the thing that scores it.
//
// A scenario supplies three facts -- where its hidden suite lives, its turn order,
// and which turn makes each group reachable -- and gets the whole runner back.
...
export function runNodeTest(dir, target) {
...
export function makeGrader({ hiddenDir, turnIds, groupEnablingTurn }) {
  function hiddenTestNames() {
    return readdirSync(hiddenDir).filter((f) => f.endsWith(".test.mjs")).sort()
  }
  ...
  function reachableGroups(turnsRun = turnIds.length) {
    return Object.entries(groupEnablingTurn)
      .filter(([, turnId]) => {
  ...
  function gradeHidden(dir, { turnsRun = turnIds.length } = {}) {
    ...
    for (const name of names) {
      copyFileSync(join(hiddenDir, name), join(target, name))
  ...
  return { hiddenTestNames, reachableGroups, gradeHidden, gradeVisible }
}
