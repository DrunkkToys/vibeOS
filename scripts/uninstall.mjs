...
function unregisterFromConfigFile(ocConfigPath) {
  ...
  // Older OpenCode builds populated the mode dropdown from a top-level `mode`
  // block instead of `agent`. Strip vibe entries there too so the dropdown
  // entry actually disappears on those hosts.
  if (config.mode && typeof config.mode === "object" && !Array.isArray(config.mode)) {
    for (const name of Object.keys(config.mode)) {
      if (VIBE_AGENTS.includes(name)) {
        delete config.mode[name]
        changed = true
      }
    }
    if (Object.keys(config.mode).length === 0) delete config.mode
  }
...
// OpenCode also builds its agent/mode dropdown from markdown agent files in
// <home>/agent/ and <project>/.opencode/agent/. Remove only vibe-named files
// that are vibeOS's own (marker in the body); a hand-written vibe.md is left.
function removeAgentMarkdown(dir) {
  let removed = false
  for (const agentDir of [join(dir, "agent"), join(dir, ".opencode", "agent")]) {
    for (const entry of safeReaddir(agentDir)) {
      if (!/^vibe(-[a-z0-9]+)?\.md$/i.test(entry)) continue
      const path = join(agentDir, entry)
      let body = ""
      try { body = readFileSync(path, "utf8") } catch { continue }
      if (!/vibeOS|VibeUltraX/i.test(body)) continue
      if (rmQuiet(path)) removed = true
    }
    if (existsSync(agentDir) && safeReaddir(agentDir).length === 0) rmQuiet(agentDir)
  }
  return removed
}
...
if (removeAgentMarkdown(process.cwd())) {
  writeLine("✓ removed vibeOS markdown agent file(s) from this project")
  didSomething = true
}
