...
function getVibeOSHome() {
  return process.env.VIBEOS_HOME || join2(process.env.HOME || homedir(), ".claude");
}
...
function writeSelection(key, value) {
  const TIERS_FILE3 = join2(getVibeOSHome(), "model-tiers.json");
  try {
...
function loadSessionSlot(sid) {
  const BLACKBOX_FILE = join2(getVibeOSHome(), "blackbox-state.json");
  try {
...
var init_selection_manager = __esm({
  "src/lib/selection-manager.js"() {
    ...
    _selCache = null;
    _selLastStamp = "";
    TIERS_FILE_PATH = () => join2(getVibeOSHome(), "model-tiers.json");
  }
});
...
function hasBypassFlag(command) {
  const c = String(command || "");
  return /--no-verify|--force|--skip-hooks|--admin|--bypass/.test(c);
}
function targetsProtectedBranch(command) {
  const c = String(command || "");
...
function resolveVibeOSHome() {
  return process.env.VIBEOS_HOME || join3(process.env.HOME || USER_HOME2, ".claude");
}
function getVibeOSHome2() {
  return VIBEOS_CONTEXT.getStore()?.home || process.env.VIBEOS_HOME || join3(process.env.HOME || "", ".claude");
}
function hasOpenCodeConfig(dir) {
  return existsSync2(join3(dir, "opencode.json")) || existsSync2(join3(dir, "opencode.jsonc"));
}
function resolveOpenCodeHomes() {
  ...
  const desktopHome = process.env.VIBEOS_OPENCODE_DESKTOP_HOME || (process.platform === "darwin" ? join3(base, "Library", "Application Support", "ai.opencode.desktop") : null);
  const configHome = join3(base, ".config", "opencode");
  const dotHome = join3(base, ".opencode");
...
function resolveOpenCodeHome() {
  ...
  return homes[0] || join3(process.env.HOME || USER_HOME2, ".config", "opencode");
}
...
function syncVibeOSPathBindings(home = resolveVibeOSHome()) {
  ...
  FILE_LOCK_DIR = join3(VIBEOS_HOME, ".vibeOS-locks");
  DELEGATION_STATE_FILE = join3(VIBEOS_HOME, "delegation-state.json");
  SAVINGS_LEDGER_FILE = join3(VIBEOS_HOME, "savings-ledger.jsonl");
  GLOBAL_LEARNING_FILE = join3(VIBEOS_HOME, "global-learning.json");
  PRICING_CACHE_FILE = join3(VIBEOS_HOME, "model-pricing-cache.json");
  BLACKBOX_STATE_FILE = join3(VIBEOS_HOME, "blackbox-state.json");
  PROJECT_STATE_FILE = join3(VIBEOS_HOME, "project-states.json");
  TIERS_FILE = join3(VIBEOS_HOME, "model-tiers.json");
  ACTIVE_JOBS_FILE = join3(VIBEOS_HOME, "active-jobs.json");
  CREDIT_CACHE_F = join3(VIBEOS_HOME, "credit-snapshot.json");
  FLOW_TODO_QUEUE_FILE = join3(VIBEOS_HOME, ".flow-todo-queue.jsonl");
  FLOW_DEDUP_FILE = join3(VIBEOS_HOME, ".flow-dedup-keys.json");
  ENFORCEMENT_COOLDOWN_FILE = join3(VIBEOS_HOME, ".enforcement-cooldown.jsonl");
  TODOS_FILE = join3(VIBEOS_HOME, "todos.json");
  REPORTS_DIR = join3(VIBEOS_HOME, "reports");
  CONTEXT7_INSTALL_FLAG = join3(VIBEOS_HOME, ".context7-install-suggested");
  TRINITY_OPENCODE_CONFIG = join3(OPENCODE_HOME, "opencode.json");
  TRINITY_OPENCODE_CONFIGC = join3(OPENCODE_HOME, "opencode.jsonc");
  SCRATCHPAD_ROOT = join3(VIBEOS_HOME, "scratch");
  SCRATCHPAD_GLOBAL_DIR = join3(SCRATCHPAD_ROOT, "by-hash");
  SCRATCHPAD_SESSIONS_DIR = join3(SCRATCHPAD_ROOT, "sessions");
}
...
function _pruneCorruptionBackups(backupDir) {
  try {
    ...
    const backups = readdirSync(backupDir).map((name) => {
      const path = join3(backupDir, name);
      try {
...
function runStartupMaintenanceOnce() {
  try {
    ...
    _pruneCorruptionBackups(join3(home, ".backups"));
    loadActiveJobs();
    updateState((state) => {
...
function _handleStateCorruption2(path) {
  ...
  const backupDir = join3(VIBEOS_HOME, ".backups");
  try {
    mkdirSync(backupDir, { recursive: true });
  } catch {
  }
  const backupPath = join3(backupDir, basename2(path) + ".corrupted." + Date.now());
  try {
    copyFileSync(path, backupPath);
  } catch {
  }
  const logPath = join3(VIBEOS_HOME, ".state-corruption-log.jsonl");
  try {
...
function updateState(mutator) {
  const delegationStateFile = join3(getVibeOSHome2(), "delegation-state.json");
...
function loadTierRegexes() {
  try {
    const p = join3(getVibeOSHome2(), "model-tiers.json");
    if (!existsSync2(p))
...
function loadGlobalLearning() {
  const globalLearningFile = join3(getVibeOSHome2(), "global-learning.json");
  try {
...
function loadBlackboxState() {
  const blackboxFile = join3(getVibeOSHome2(), "blackbox-state.json");
  try {
...
function safeCopyIntoSession(hash, fromPath, targetScratchpadDir = getSessionScratchpadDir()) {
  try {
    ...
    const sessionPath = join3(targetScratchpadDir, `${hash}.txt`);
    if (!existsSync2(sessionPath)) {
      copyFileSync(fromPath, sessionPath);
      const globalSummary = join3(SCRATCHPAD_GLOBAL_DIR, `${hash}.summary.txt`);
      const sessionSummary = join3(targetScratchpadDir, `${hash}.summary.txt`);
...
`).digest("hex").slice(0, 16);
  ...
  try {
    ...
    if (ageSec > SCRATCHPAD_MAX_AGE_SEC)
      return null;
    const summaryPath = join3(sessionDir, `${hash}.summary.txt`);
...
function pruneScratchpadOnce() {
  ...
  try {
    const script = join3(VIBEOS_HOME, "hooks/scratchpad-prune.sh");
...
function loadProjectState() {
  const projectStateFile = join3(getVibeOSHome2(), "project-states.json");
  try {
...
function detectTechStack(dir) {
  const stacks = [];
  try {
    const pkg = safeJsonParse2(readFileSync2(join3(dir, "package.json"), "utf-8"));
    if (pkg) {
      if (pkg.devDependencies?.typescript || pkg.dependencies?.typescript || existsSync2(join3(dir, "tsconfig.json")))
  ...
  try {
    if (existsSync2(join3(dir, "Cargo.toml")))
      stacks.push("rust");
  } catch {
  }
  try {
    if (existsSync2(join3(dir, "go.mod")))
      stacks.push("go");
  } catch {
  }
  try {
    if (existsSync2(join3(dir, "requirements.txt")))
      stacks.push("python");
    if (existsSync2(join3(dir, "setup.py")))
      stacks.push("python");
    if (existsSync2(join3(dir, "pyproject.toml")))
      stacks.push("python");
  } catch {
  }
...
var init_state = __esm({
  "src/lib/state.js"() {
    ...
    TIERS_FILE = join3(VIBEOS_HOME, "model-tiers.json");
    ACTIVE_JOBS_FILE = join3(VIBEOS_HOME, "active-jobs.json");
    AUTH_F = join3(USER_HOME2, ".local", "share", "opencode", "auth.json");
    CREDIT_CACHE_F = join3(VIBEOS_HOME, "credit-snapshot.json");
    FLOW_TODO_QUEUE_FILE = join3(VIBEOS_HOME, ".flow-todo-queue.jsonl");
    FLOW_DEDUP_FILE = join3(VIBEOS_HOME, ".flow-dedup-keys.json");
    ENFORCEMENT_COOLDOWN_FILE = join3(VIBEOS_HOME, ".enforcement-cooldown.jsonl");
...
var DEFAULT_API_URL, EMBEDDED_API_TOKEN, API_TOKEN_RE, API_DISABLED_RE, REQUEST_TIMEOUT, MAX_RETRIES, BASE_RETRY_DELAY, ALPHA_BUILD_CHANNEL, BOOTSTRAP_EXCHANGE_PATH, BOOTSTRAP_RETRY_COOLDOWN_MS, VibeOSAuthError, VibeOSTimeoutError, VibeOSNetworkError, ANOMALY_BURST_WINDOW_MS, ANOMALY_BURST_THRESHOLD, ANOMALY_FREQ_WINDOW_MS, ANOMALY_STDDEV_FACTOR, ANOMALY_WARMUP_MS, ANOMALY_COOLDOWN_MS, TokenAnomalyDetector, VibeOSApiClient, VIBEOS_API_URL, _apiDir, _vibeHome, _envPaths, _bootstrapEnvPath, VIBEOS_API_DISABLED, VIBEOS_API_TOKEN, VIBEOS_API_BOOTSTRAP_TOKEN, VIBEOS_API_ENABLED, _anomalyDetector, _apiClientHolder, _apiClientGen, _apiFallbackMode, _apiFallbackSince, _bootstrapExchangeInFlight, _bootstrapExchangeFailedAt, _backendVersion, _apiLatencyDegradedUntil, LATENCY_GUARDED_METHODS, FALLBACK_COOLDOWN_MS, LATENCY_DEGRADE_THRESHOLD_MS, LATENCY_DEGRADE_COOLDOWN_MS;
var init_api_client = __esm({
  "src/lib/api-client.js"() {
    "use strict";
    init_runtime_state();
    DEFAULT_API_URL = "https://api.vibetheog.com";
    EMBEDDED_API_TOKEN = "vos_8d73804b13bb46711b9a47f036dba7b4d026fd9583d96960e663716e62815a69";
    API_TOKEN_RE = /^vos_[a-f0-9]{64}$/i;
    API_DISABLED_RE = /^(1|true|yes|on)$/i;
    REQUEST_TIMEOUT = 1e4;
    MAX_RETRIES = 3;
    BASE_RETRY_DELAY = 1e3;
    ALPHA_BUILD_CHANNEL = String(process.env.VIBEOS_BUILD_CHANNEL || "alpha").toLowerCase();
    BOOTSTRAP_EXCHANGE_PATH = "/api/v1/auth/bootstrap/exchange";
    BOOTSTRAP_RETRY_COOLDOWN_MS = 6e4;
    VibeOSAuthError = class extends Error {
      statusCode;
      code;
    ...
    VibeOSTimeoutError = class extends Error {
      constructor(message) {
        super(message);
        this.name = "VibeOSTimeoutError";
      }
    };
    VibeOSNetworkError = class extends Error {
      constructor(message) {
        super(message);
        this.name = "VibeOSNetworkError";
      }
    };
    ANOMALY_BURST_WINDOW_MS = 5e3;
    ANOMALY_BURST_THRESHOLD = 10;
    ANOMALY_FREQ_WINDOW_MS = 6e5;
    ANOMALY_STDDEV_FACTOR = 3;
    ANOMALY_WARMUP_MS = 3e4;
    ANOMALY_COOLDOWN_MS = 12e4;
    TokenAnomalyDetector = class {
      burstHistory = [];
      freqHistory = [];
      lastWarnTime = 0;
    ...
    VIBEOS_API_URL = process.env.VIBEOS_API_URL || "https://api.vibetheog.com";
    _apiDir = typeof __dirname !== "undefined" ? __dirname : dirname4(fileURLToPath3(import.meta.url));
    _vibeHome = process.env.VIBEOS_HOME || join(process.env.HOME || homedir3(), ".claude");
    _envPaths = [_vibeHome, _apiDir, process.cwd(), homedir3()];
    _bootstrapEnvPath = _envPaths[0] + "/.env.alpha";
...
var init_pivot_cache = __esm({
  "src/vibeOS-lib/blackbox/pivot-cache.js"() {
    ...
    PivotCache = class {
      ...
      lastTokens;
      constructor(baseDir) {
        this.baseDir = baseDir || join7(homedir5(), ".claude");
        this.pivotSequence = [];
      ...
      _storePath() {
        return join7(this.baseDir, ".vibeos-pivot-cache.json");
      }
      _indexPath() {
        return join7(this.baseDir, ".vibeos-pivot-cache.index.json");
      }
      _loadStore() {
        try {
...
function getRealityCheckSettingsFile() {
  return join4(getVibeOSHome3(), "reality-check-settings.json");
}
...
var RULES_PATH_CANDIDATES = [
  join4(process.cwd(), "src", "vibeOS-lib", "flow-rules.json"),
  join4(process.cwd(), "dist-ts", "vibeOS-lib", "flow-rules.json"),
  join4(process.cwd(), "dist", "assets", "flow-rules.json"),
  join4(__dirname2, "flow-rules.json")
];
...
function getStateFile() {
  return join4(getVibeOSHome3(), "delegation-state.json");
}
function getFlowTodoFile() {
  return join4(getVibeOSHome3(), ".flow-todo-queue.jsonl");
}
var FLOW_DEDUP_FILE2 = join4(getVibeOSHome3(), ".flow-dedup-keys.json");
...
import { extname, join as join5, dirname as dirname3 } from "node:path";
...
function resolveDashboardDir() {
  const repoRoot = join5(_MCP_DIR, "..", "..");
  const cwd = process.cwd();
  const c = [
    join5(_MCP_DIR, "dashboard", "dist"),
    join5(_MCP_DIR, "assets", "dashboard"),
    join5(_MCP_DIR, "assets", "dashboard", "dist"),
    join5(repoRoot, "src", "lib", "dashboard", "dist"),
    join5(cwd, "src", "lib", "dashboard", "dist"),
    join5(cwd, "dist-ts", "lib", "dashboard", "dist")
  ];
  for (const p of c) {
    if (existsSync4(join5(p, "index.html")))
      return p;
  }
  return c[0];
}
var DASHBOARD_DIR = resolveDashboardDir();
var DASHBOARD_CONFIG_PATH = join5(DASHBOARD_DIR, "vibeos-dashboard-config.js");
...
function serveDashboard(res, p) {
  const idx = join5(DASHBOARD_DIR, "index.html");
  let fp2 = join5(DASHBOARD_DIR, p === "/" ? "index.html" : p);
...
import { join as join6, dirname as dirname5, resolve } from "node:path";
...
function getVibeOSHome4() {
  return process.env.VIBEOS_HOME || join6(process.env.HOME || homedir4(), ".claude");
}
function getOpenCodeDesktopHome() {
  return process.env.VIBEOS_OPENCODE_DESKTOP_HOME || join6(process.env.HOME || homedir4(), "Library", "Application Support", "ai.opencode.desktop");
}
var TIERS_FILE2 = join6(getVibeOSHome4(), "model-tiers.json");
function _lockPathFor2(filePath) {
  const hash = createHash3("sha1").update(String(filePath || "")).digest("hex");
  return join6(getVibeOSHome4(), ".vibeOS-locks", `${hash}.lock`);
}
function withFileLock2(filePath, fn, opts = {}) {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      mkdirSync5(join6(getVibeOSHome4(), ".vibeOS-locks"), { recursive: true });
...
function _loadDynamicPricingCache() {
  ...
  const PRICING_CACHE_FILE2 = join6(home, "model-pricing-cache.json");
  try {
...
function _writeDynamicPricingCache(modelsMap) {
  if (!modelsMap || typeof modelsMap !== "object")
    return;
  const PRICING_CACHE_FILE2 = join6(getVibeOSHome4(), "model-pricing-cache.json");
  try {
...
function _loadPricingOverrides() {
  ...
  try {
    const tiersFile = join6(home, "model-tiers.json");
    ...
    _handleStateCorruption2(join6(home, "model-tiers.json"));
    _pricingOverridesCache = {};
  }
  return _pricingOverridesCache;
}
...
var CONTEXT7_CONFIG_FILES = [
  join6(getVibeOSHome4(), "settings.json"),
  join6(getVibeOSHome4(), ".claude.json"),
  join6(getOpenCodeHome(), "opencode.json"),
  join6(process.cwd(), "opencode.json")
];
  ...
  try {
    ...
    for (const entry of readdirSync2(baseDir)) {
      if (!entry.endsWith(".json"))
        continue;
      const full = join6(baseDir, entry);
  ...
  try {
    ...
    for (const dir of pathDirs) {
      if (!dir)
        continue;
      try {
        if (existsSync6(join6(dir, "context7")))
          return true;
        if (existsSync6(join6(dir, "context7.cmd")))
          return true;
      } catch {
      }
    }
...
function _context7InNpmCache() {
  try {
    const npxDir = join6(USER_HOME3, ".npm/_npx");
    if (!existsSync6(npxDir))
      return false;
    for (const hashDir of readdirSync2(npxDir)) {
      const ctxDir = join6(npxDir, hashDir, "node_modules", "context7");
      try {
        if (existsSync6(join6(ctxDir, "package.json")))
          return true;
      } catch {
      }
    }
  ...
  for (const root of roots) {
    try {
      if (!existsSync6(root) || !statSync5(root).isDirectory())
        continue;
      const files = readdirSync2(root).filter((name) => /^opencode\.workspace\..*\.dat$/i.test(name)).map((name) => join6(root, name)).sort((a, b) => statSync5(b).mtimeMs - statSync5(a).mtimeMs);
...
function loadTrinitySlotsFromTiersFile() {
  try {
    const TIERS_FILE3 = join6(getVibeOSHome4(), "model-tiers.json");
...
import { join as join8, dirname as dirname8 } from "node:path";
...
function loadTrinityModels() {
  try {
    const p = join8(getVibeOSHome5(), "model-tiers.json");
    if (!existsSync9(p))
...
function computeLocalCalibration() {
  try {
    const calFile = join8(getVibeOSHome5(), "calibration-data.jsonl");
...
var SCRATCHPAD_ROOT2 = join9(getVibeOSHome6(), "scratch");
var SCRATCHPAD_GLOBAL_DIR2 = join9(SCRATCHPAD_ROOT2, "by-hash");
var SCRATCHPAD_SESSIONS_DIR2 = join9(SCRATCHPAD_ROOT2, "sessions");
var STATE_FILE = join9(getVibeOSHome6(), "delegation-state.json");
var currentModel2 = null;
function getSessionRoot2() {
  return join9(SCRATCHPAD_SESSIONS_DIR2, _OC_SID2);
}
function getSessionScratchpadDir2() {
  return join9(getSessionRoot2(), "by-hash");
}
function getGlobalIndexPath2() {
  return join9(SCRATCHPAD_ROOT2, "index.jsonl");
}
  ...
  try {
    ...
    if (existsSync10(indexPath)) {
      ...
      for (const line of lines) {
        ...
        const hash = e.hash;
        const summaryPathSession = join9(getSessionScratchpadDir2(), hash + ".summary.txt");
        const summaryPathGlobal = join9(SCRATCHPAD_GLOBAL_DIR2, hash + ".summary.txt");
...
function getReportsDir() {
  return join10(getVibeOSHome7(), "reports");
}
function getReportsIndexPath() {
  return join10(getReportsDir(), "index.json");
}
  ...
  try {
    ...
    for (const r of idx.reports) {
      ...
      if (now - created > 90 * 24 * 3600 * 1e3) {
        try {
          rmSync4(join10(getReportsDir(), `${r.id}.json`));
        } catch {
        }
        continue;
      }
...
function readReport(id2) {
  ...
  if (!/^[\w-]+$/.test(String(id2)))
    return null;
  const path = join10(getReportsDir(), `${id2}.json`);
  try {
...
function getVibeOSHome8() {
  return process.env.VIBEOS_HOME || join11(process.env.HOME || "", ".claude");
}
...
function _cachedPct() {
  try {
    ...
    let budget = 50;
    try {
      const p = join11(getVibeOSHome8(), "model-tiers.json");
      if (existsSync12(p)) {
...
function loadCredit() {
  ...
  try {
    const f = join11(getVibeOSHome8(), "credit-percent");
    if (existsSync12(f)) {
  ...
  return {
    ...
    async execute({ action, slot, level, model, token } = {}) {
      ...
      if (action === "reality-check") {
        ...
        const realityFile = join12(deps.VIBEOS_HOME || process.env.VIBEOS_HOME || join12(process.env.HOME || "", ".claude"), "reality-check-settings.json");
        const stateFile = deps.STATE_FILE;
        const projectStateFile = join12(deps.VIBEOS_HOME || process.env.VIBEOS_HOME || join12(process.env.HOME || "", ".claude"), "project-states.json");
...
${L.repeat(40)}`);
      ...
      if (action === "verify-claims") {
        const VIBEOS_HOME2 = process.env.VIBEOS_HOME || join12(process.env.HOME || "", ".claude");
        const AUDIT_DIR = join12(VIBEOS_HOME2, "cascade-audit");
        const claimFile = join12(AUDIT_DIR, "claim-audit.jsonl");
        const cascadeFile = join12(AUDIT_DIR, "cascade-audit.jsonl");
      ...
      if (action === "diagnose") {
        const results = [];
        const ocConfig = join12(deps.OPENCODE_HOME, "opencode.json");
        const apiFallbackActive = typeof deps.isApiFallback === "function" ? deps.isApiFallback() : false;
        const checks = [
      ...
      if (action === "repair-state") {
        ...
        for (const r of idx.reports || []) {
          if (r.project !== name || r.fingerprint !== dstFp)
            continue;
          const rf = join12(deps.REPORTS_DIR, `${r.id}.json`);
          try {
...
function evaluateClaimVerification({ text, vibeHome = process.env.VIBEOS_HOME || join14(process.env.HOME || "", ".claude"), now = Date.now(), windowMs = 12e4 }) {
...
function deriveRole(toolName, input, output) {
  ...
  if (typeof cmd !== "string")
    return "query";
  if (hasBypassFlag(cmd))
    return "bypass";
  if (isDeployCommand(cmd))
    return "deployment";
  const family = commandFamily(cmd);
  if (["git-status", "syntax-check", "typecheck", "test", "build"].includes(family))
    return "verification";
  return "query";
}
...
function getSessionEventLogPath(sid) {
  const dir = join15(VIBEOS_HOME, "session-events");
  mkdirSync10(dir, { recursive: true });
  return join15(dir, sid + ".jsonl");
}
  ...
  try {
    ...
    updateState((s) => {
      ...
      try {
        ...
        if (sd) {
          const sp = join16(sd, "delegation-state-hint.txt");
          try {
...
`;
function installVibeSkill(targetRoot) {
  ...
  mkdirSync11(dirname10(skillPath), { recursive: true });
  writeFileSync14(skillPath, VIBE_SKILL_BODY, "utf8");
...
function ensureVibeSkill(dir) {
  try {
    return installVibeSkill(dir);
  } catch (err) {
    console.error(`[vibeOS] Project Guard: failed to create /vibe skill for ${basename3(dir || "") || "unknown"}: ${err.message}`);
...
function resolveRestorableOpenCodeAgent(currentSel) {
  ...
  if (remembered && remembered !== "plan")
    return remembered;
  try {
    const configDir = dirname11(TRINITY_OPENCODE_CONFIG || join18(getOpenCodeHome(), "opencode.json"));
    const candidates = readdirSync3(configDir).filter((name) => /^opencode\.json\.bak/.test(name)).map((name) => {
      const path = join18(configDir, name);
...
function ensureProjectSkill(dir, fp2) {
  ...
  const skillsDir = join18(dir, ".opencode", "skills");
...
`;
  ...
  try {
    ...
    writeFileSync15(skillPath, content, "utf-8");
...
function syncControlSettings(cv, options = {}) {
  ...
  try {
    ...
    if (cv.agent_mode) {
      try {
        const OC_CONFIG = TRINITY_OPENCODE_CONFIG || join18(getOpenCodeHome(), "opencode.json");
        if (existsSync17(OC_CONFIG)) {
          ...
          if (oc.default_agent !== cv.agent_mode) {
            ...
            writeFileSync15(OC_CONFIG, JSON.stringify(oc, null, 2) + "\n");
          }
        }
      } catch {
      }
...
`).digest("hex").slice(0, 16);
      ...
      const sessPath = join18(getSessionScratchpadDir(), `${hash}.txt`);
      const globalPath = join18(globalDir, `${hash}.txt`);
      try {
...
`).digest("hex").slice(0, 16);
          ...
          try {
            writeFileSync15(ptrPath, JSON.stringify({ contentHash: hash, tool: part.tool }));
          } catch {
          }
        }
      } catch (err) {
  ...
  try {
    ...
    try {
      if (!Array.isArray(messages) || Object.isFrozen(messages))
        return;
      const vibeHome = process.env.VIBEOS_HOME || join18(process.env.HOME || homedir6(), ".claude");
...
var onSystemTransform = async (_input, output) => {
  ...
  try {
    ...
    if (_turnCountInject % 10 === 0 && _calBuffer.length > 0) {
      try {
        const calFile = join18(getVibeOSHome9(), "calibration-data.jsonl");
        mkdirSync12(getVibeOSHome9(), { recursive: true });
...
// src/vibeOS-lib/reward-engine.js
var REWARD_TABLE = {
  qualityReward: 10,
  savingBonusMax: 3,
  savingThresholdSmall: 1e-3,
...
// src/vibeOS-lib/laziness-detector.js
var TODO_RE = /\b(TODO|FIXME|PLACEHOLDER|TBD|XXX|lorem ipsum)\b/i;
var SHORT_THRESHOLD = 100;
function detectLaziness(input) {
...
function loadSelection2() {
  try {
    const raw = readFileSync17(join19(getVibeOSHome10(), "model-tiers.json"), "utf-8");
      ...
      try {
        ...
        if (_prevOutputText && prevText && _prevOutputText !== prevText) {
          ...
          if (finalOutcome) {
            ...
            try {
              mkdirSync13(getVibeOSHome10(), { recursive: true });
              appendFileSync5(join19(getVibeOSHome10(), "calibration-data.jsonl"), JSON.stringify({ ts: (/* @__PURE__ */ new Date()).toISOString(), event: "outcome", sid: getSessionId(), outcome: finalOutcome }) + "\n");
            } catch {
            }
          }
        }
...
var ENFORCEMENT_LOCK_DIR = join20(getVibeOSHome11(), ".enforcement-lock");
var LOCK_EXPIRE_MS = 3e4;
var ENFORCEMENT_COOLDOWN_FILE2 = join20(getVibeOSHome11(), ".enforcement-cooldown.jsonl");
var COOLDOWN_MS = 6e4;
...
function buildTestSkeleton(filePath, sourceContent = "", options = {}) {
  ...
  return { path: testPath, content: skeletonFn(name, exports, "full", strict, quality, sourceContent, fw?.framework), dir: dirname12(testPath) };
}
...
function _resolveToolPath(pathValue) {
  ...
  if (raw.startsWith("/"))
    return raw;
  return projectDirectory ? join21(projectDirectory, raw).replace(/\\/g, "/") : raw;
}
...
function _prependFooterAlert(target, footerText, seen = /* @__PURE__ */ new Set()) {
  ...
  for (const key of ["message", "data", "payload", "output", "result"]) {
    if (_prependFooterAlert(target[key], footerText, seen))
      return true;
  }
  for (const value of Object.values(target)) {
    if (_prependFooterAlert(value, footerText, seen))
      return true;
  }
  return false;
}
...
`).digest("hex").slice(0, 16);
  ...
  try {
    ...
    const ptrPath = join21(sessionDir, `${hash}.ptr`);
...
var onToolExecuteAfter = async (input, output) => {
  ...
  try {
    if (t !== "task") {
      ...
      if (!liveModel) {
        liveModel = readConfig(projectDirectory) || readConfig(join21(process.env.HOME || "", ".config", "opencode")) || process?.env?.OPENCODE_MODEL || "";
      }
  ...
  if (t === "trinity") {
    ...
    if (trinityAction === "todo") {
      try {
        const flowTodoFilePath = join21(getVibeOSHome12(), ".flow-todo-queue.jsonl");
        let todoLines = [];
...
${pendingUiNote}`;
  ...
  observeToolPattern2(t, input, output, projectDirectory);
  if (t === "task") {
...
function getVibeOSHome13() {
  return process.env.VIBEOS_HOME || join22(process.env.HOME || "", ".claude");
}
function getTiersFile() {
  return join22(getVibeOSHome13(), "model-tiers.json");
}
function getReportsDir2() {
  return join22(getVibeOSHome13(), "reports");
}
function getMcpRuntimeFile() {
  return join22(getVibeOSHome13(), "mcp-runtime.json");
}
...
function scanClaimsInOutput(output) {
  ...
  try {
    ...
    const auditDir = join22(getVibeOSHome13(), "cascade-audit");
...
function _loadActiveJobForProject(directory3, fp2 = "") {
  const candidates = [getVibeOSHome13(), directory3 ? join22(directory3, "..") : ""].filter(Boolean);
...
export {
  ...
  modelToCcAlias,
  noteProjectPattern,
  observeToolPattern2 as observeToolPattern,
  readReport,
  recordSaving,
  remoteCall,
  researchAudit,
