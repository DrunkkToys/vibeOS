...
This AGENTS.md is the master index for the vibeOS ecosystem. Five repositories form the complete stack:

| Project | Role | npm / Location | Repo |
|---------|------|----------------|------|
| **vibeOS** (this) | OpenCode plugin | `vibeostheog` npm | [github.com/DrunkkToys/vibeOS](https://github.com/DrunkkToys/vibeOS) |
| **vibeOScore** | API server + MCP + dashboard | `vibescore` npm | [github.com/DrunkkToys/vibeOScore](https://github.com/DrunkkToys/vibeOScore) |
| **vibeOSmcp** | Standalone MCP server | separate npm | [github.com/DrunkkToys/vibeOSmcp](https://github.com/DrunkkToys/vibeOSmcp) |
| **VibeBrainUltra** | ML training / cascade router | Python | [github.com/DrunkkToys/VibeBrainUltra](https://github.com/DrunkkToys/VibeBrainUltra) |
| **VibeTheOG** | Legacy deploy / config | Node + PHP | [github.com/DrunkkToys/VibeTheOG](https://github.com/DrunkkToys/VibeTheOG) |

**Dev machine layout:** Sibling directories under ~/Desktop/. If you are NOT on the dev machine, follow GitHub URLs above.

**Key relationships:**
- vibeOS depends on vibeOScore API (remote or local)
- vibeOSmcp wraps vibeOScore API as MCP protocol
- VibeBrainUltra trains the theOG cascade router used by vibeOScore
- VibeTheOG contains legacy deployment and chat frontend
...
6. **TDD enforcer** — Auto skeleton tests for changed source files. Strict mode (ON by default): TODO tests fail loudly. Quality mode: `vibe tdd quality on|off`.
...
14. **Pattern learner and runtime controls** — Learns recurring struggle/routine patterns per project and exposes `vibe patterns` and `vibe patterns clear`.
...
- **Remote API server:** Lives in **vibeOScore** (`../vibeOScore/src/` on dev machine, or deployed at `api.vibetheog.com`). The plugin client (`src/lib/api-client.ts`) connects to it with automatic local fallback. Do NOT create API server files in this repository.
...
### Dashboard Build Chain (lives in vibeOScore)

The dashboard lives in `../vibeOScore/dashboard/` (on dev machine). SolidJS SPA built with Vite. Compiled output served by MCP server. Do NOT create dashboard files in this repository.
...
Tests are **auto-discovered** from `tests/`, `src/tests/`, `src/utils/tests/`, `src/lib/hooks/tests/`, and `src/vibeOS-lib/tests/`.
Any `.test.mjs` or `.test.js` file placed in those directories is picked up automatically by `scripts/run-test-suite.mjs`.
No CI config updates needed when adding new test files.

Key suites (non-exhaustive — new files are auto-discovered):

| Suite | File | Tests | Coverage |
|-------|------|-------|----------|
| **Forensic cascade** | `forensic_cross_cascade.test.mjs` | 10 | `.claude` as file, concurrent tiers, slot lock, ledger, stale jobs, state reload, reports, stress/classify, bootstrap token, enforcement block |
| **Mega adversarial** | `mega/mega_*.test.mjs` | 39 | 10 suites: bootstrap, enforcement matrix, state corruption, concurrency, footer, API fallback, blackbox, flow/TDD, savings, stress |
| **Deep integration** | `deep_integration.test.mjs` | 815 | Pipeline modes, vibelitex recovery, syncControlSettings, cold start, concurrent slots |
| **Delegation enforcer** | `test_delegation_enforcer.test.mjs` | 141 | Enforcement logic, credit nudges, warning coalescing |
| **Cross-module integration** | `integration_cross_module.test.mjs` | 52 | Slot preservation, ML router, stress, crash guards |
| **Production regressions** | `production_regressions.test.mjs` | 35 | Mode compat, footer, cross-version |
| **E2E lifecycle** | `e2e-plugin-lifecycle.test.mjs` | 5 | Hooks wiring: system transform, messages transform, tool.before, tool.after |
| **Install + recovery** | `test_install_and_recovery.test.mjs` | 14 | Schema upgrade, bare machine, .claude as file |
| **Agent mode** | `test_agent_mode_integration.test.mjs` | 22 | Control-vector, enforcement_mode, optimization_mode |
...
## 🗣 TALK TO THE HUMAN — DO NOT LIE

- If a feature is unavailable (API down, model not found, tool blocked by delegation enforcement), **report the real reason**. Never fabricate results.
- If something cannot execute, say why. Ask permission once — do not ask again and again.
- When delegating to workers, report what was delegated and what came back.
- Limitations are features: tell the user what the system **cannot** do, so they can adapt.
- The speed guard is active. Verify before acting. Choose check-first over fix-first when ambiguous.
- If the human seems overwhelmed by your speed, slow down. Double check before overwriting. The human's understanding matters more than your velocity.

## 🔧 CROSS-DEVICE DESIGN PRINCIPLE

This is the **development** repository. Every design decision must work for users on OTHER machines.

- **Never hardcode** paths, URLs, IPs, model IDs, or credentials in code that ships to users.
- Use **env vars with sensible defaults**. Document what to override in `.env.production`.
- The AGENTS.md is the **index**, not a config file. Configuration goes in `~/.claude/model-tiers.json` and env vars.
- If the code has a dev-machine-specific fallback (like hardcoded model names as fallback when tiers are empty), mark it with `// DEV ONLY: fallback for dev machine when vibe rebuild has not been run`
- Model defaults (v4-pro, v4-flash, deepseek-chat) are ONLY correct for the dev machine. Users MUST run `vibe rebuild` to populate their own model slots.
- API URL defaults (api.vibetheog.com) can be overridden via `VIBEOS_API_URL` env var.
