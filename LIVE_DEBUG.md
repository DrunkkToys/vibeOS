...
> **⚠ YOU ARE NOW A QA TESTER.** Reading this file makes you responsible for executing **every single prompt** in this document against a live OpenCode session. Do NOT skip prompt-based tests or substitute with `node --test` runs. Structural checks (syntax, typecheck) are supplements, not replacements. Each prompt must be sent, the result inspected, and the Expected section verified. If any test fails, the release is BLOCKED.
...
python3 -c "exec(open('LIVE_DEBUG.md').read().split('### 21.9')[1].split('```bash')[1].split('```')[0].strip().split('python3 -c')[1].split(\"'\")[1])" 2>/dev/null || echo "SKIP: run Section 20.9 manually"
...
**Expected:**
- Footer format: `— [modelTag][VIBE→MODE][ENF ON] | vibeOS: $X.XX saved arrow —` (tags vary by mode, enforcement, and blackbox state)
- `vibe status` output shows:
...
- After the 10th suppressed warn, a coalesce message: `"<category>: 10 warnings coalesced — vibe medium recommended"`
...
**Prompt:**
```bash
VIBEOS_API_ENABLED=true VIBEOS_API_TOKEN=vos_invalid_token vibe status
```
...
- Footer or vibe status may indicate fallback mode (e.g., `[LOCAL]`).
...
**Expected:**
- `vibe tdd on` confirms enforcement active.
...
**Why:** Tests vibe commands toggle the blackbox engine, and when disabled, `classifyTurnSimple()` fallback is used instead.
...
**Prompt:**
> FIX THIS SHIT RIGHT NOW. This is COMPLETELY BROKEN and WRONG. FIX IT!!! Write a script that merges CSV files. I need this working IMMEDIATELY. After you finish, run `vibe status` and report the MODE, stress level, and whether quality mode was activated.
...
- `vibe status` shows MODE = quality.
...
- `vibe patterns` shows detected pattern.
...
- After 3+ sessions with same pattern, it's promoted and visible via `vibe patterns suggest`.
...
- `vibe patterns` returns "no patterns learned yet".
...
**Why:** Tests the `vibe guard` command regenerates AGENTS.md and README.md on demand.
...
- `vibe status` reflects cheap slot.
...
### 11.11 VIBE Auto-Select Mode (Regime + Stress)
...
## 16. VIBE FOOTER & BLACKBOX TDD CONTROL

> **CRITICAL:** These tests verify the VIBE→ tag replaces AUTO→ and that blackbox controls TDD on/off, not hardcoded mode checks.

### 16.1 VIBE Footer Tag — AUTO replaced by VIBE

**Prompt:**
> vibe status

**Why:** Tests that the footer shows `[VIBE→` instead of the old `[AUTO→` tag when auto-mode is active. If the mode was set explicitly (no auto), only the mode tag like `[BUDGET]` appears without the VIBE prefix.

**Expected:**
- When auto-mode is active (blackbox enabled, regime != null): footer shows `[VIBE→BUDGET]` or `[VIBE→QUALITY]` or `[VIBE→SPEED]` depending on blackbox regime + stress analysis.
- When mode is set explicitly (`vibe set cheap` without auto): footer shows bare `[BUDGET]` with no VIBE prefix.
- The arrow character `→` is the Unicode right-arrow (U+2192), not a hyphen or dash.
- The `[VIBE` prefix is uppercase, not `[Vibe` or `[vibe`.

---

### 16.2 VIBE Footer — Budget Mode Shows No VIBE Prefix

**Prompt:**
> vibe set cheap

**Why:** Tests that when the user manually overrides to cheap tier (budget), the footer does NOT show `[VIBE→` prefix — only the bare `[BUDGET]` tag.

**Expected:**
- Footer shows `[BUDGET]` without any `[VIBE→` prefix.
- This distinguishes user-forced budget from blackbox-decided VIBE→BUDGET.

---

### 16.3 VIBE Footer — All Mode Tags Display Correctly

**Prompt:**
> vibe status
> (then inspect the footer for the current session)

**Why:** Tests that all five possible optimization mode tags display correctly in the footer: AUDIT, BUDGET, QUALITY, SPEED, LONGRUN, BALANCED.

**Expected:**
- Tags match the current optimization mode.
- Tags are uppercase, bracketed, with no extra spaces.
- When blackbox VIBE mode is active: `[VIBE→BUDGET]`, `[VIBE→QUALITY]`, etc.

---

### 16.4 Blackbox TDD Control — TDD ON in Quality Mode

**Prompt:**
> vibe set brain
> Write a file `src/test_util.ts` with `export function add(a: number, b: number): number { return a + b; }`

**Why:** Tests that when blackbox sets mode to quality (brain tier, strict enforcement), TDD enforcement is ON because blackbox's `syncControlSettings()` sets `tdd_enforce: true`.

**Expected:**
- Footer shows `[TDD ON]` (or no TDD tag if TDD is not enabled for quality mode in blackbox config).
- `model-tiers.json.selection.tdd_enforce` is `true`.
- Test skeleton file created for `src/test_util.ts`.

---

### 16.5 Blackbox TDD Control — TDD OFF in Budget Mode

**Prompt:**
> vibe set cheap

**Why:** Tests that in budget mode, blackbox sets `tdd_enforce: false` via `syncControlSettings()`. The footer should NOT show any TDD tag at all (no `[TDD ON]`, no `[TDD OFF]`).

**Expected:**
- Footer does NOT show `[TDD ON]` or `[TDD OFF]`.
- `model-tiers.json.selection.tdd_enforce` is `false`.
- No TDD skeleton created on file writes.
- This confirms blackbox is the sole authority for TDD state — no hardcoded mode check in the footer code.

---

### 16.6 Blackbox TDD Control — Blackbox Toggle Reflects in TDD

**Prompt:**
> trinity blackbox off
> Write a file `src/blackbox_off_test.ts` with `export function test() { return true; }`
> trinity blackbox on

**Why:** Tests that disabling blackbox reverts to `classifyTurnSimple()` fallback, which has its own TDD logic. Re-enabling blackbox restores full resolution tracker TDD control.

**Expected (off):**
- `_blackboxEnabled = false`.
- Footer blackbox state indicators disappear.
- TDD may be ON or OFF based on fallback classifier (not the full blackbox engine).

**Expected (on):**
- `_blackboxEnabled = true`.
- Full blackbox TDD control restored on next turn.

---

## 21. CONTEXT7 OPTIMIZATION
...
## 21. SAVINGS RECONCILIATION

### 21.1 Ledger-to-State Reconciliation

**Prompt:** Delete `delegation-state.json` but keep `savings-ledger.jsonl`, then restart plugin and run `vibe status`.
...
### 21.2 Savings Persistence Across Sessions

**Prompt:** End session (restart), start new session, check `vibe status`.
...
### 21.1 No State Files — Fresh Install
...
### 21.3 Brain Tier Direct Write — No Enforcement Disabled
...
## 21. ML ROUTER & SMART CACHE

### 21.1 Smart Cache — Predict Hit on Repeat Tool
...
### 21.3 ML Router — Compute Difficulty
...
### 21.4 ML Router — Predict Best Model
...
### 21.5 ML Router — Route Edge Learning
...
### 21.6 Smart Cache Live Verification
...
## 21. STATE CORRUPTION & RECOVERY INTEGRITY
...
### 21.1 Savings Ledger — Append Integrity (Newline Separation)
...
### 21.2 Savings Ledger — Rebuild Captures All Entries
...
### 21.3 Savings Not Lost After State Corrupt+Rebuild Cycle
...
# DO NOT actually delete here — check vibe status instead
vibe status 2>/dev/null | grep -oE '\$[0-9]+\.[0-9]+' | head -1
```
...
### 21.4 Model Pricing Cache — No Corruption Loop
...
### 21.5 Warn Dedup — Duplicate Keys Coalesced
...
### 21.6 Session Timestamp — `session_started_at` Not Null
...
### 21.7 Model Split — Non-Empty After Tool Use
...
### 21.8 State Corruption — Backup Before Overwrite
...
### 21.9 End-to-End: Session Lifecycle Integrity
