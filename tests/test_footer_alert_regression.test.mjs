...
test("footer: sticky branded mode stays visually distinct from the live regime label", async () => {
    ...
    assert.ok(footer.includes("VibeUltraX"), "footer should show the VibeUltraX brand: " + footer)
    assert.ok(!footer.includes("· VibeUltraX"), "footer must not duplicate the brand: " + footer)
})
...
test("footer: vibeultrax tier follows the live model, not the stale persistent slot", async () => {
    ...
    assert.ok(footer.includes("VibeUltraX"), "footer should show VibeUltraX brand: " + footer)
