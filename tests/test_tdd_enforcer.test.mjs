...
test("buildTestSkeleton: generates skeleton path for .ts", async () => {
  ...
  const skeleton = await buildTestSkeleton("src/worker.ts", 'export function worker() { return 42 }', { strict: false, quality: false })
...
test("buildTestSkeleton: generates skeleton for .cjs", async () => {
  const { buildTestSkeleton } = await import(join(ROOT, "src/index.js"))
  const skeleton = await buildTestSkeleton("src/util.cjs", 'module.exports = { run: () => {} }', { strict: false, quality: false })
...
test("buildTestSkeleton: generates skeleton for .mts", async () => {
  const { buildTestSkeleton } = await import(join(ROOT, "src/index.js"))
  const skeleton = await buildTestSkeleton("src/config.mts", 'export const PORT = 3000', { strict: false, quality: false })
...
test("buildTestSkeleton: strict mode includes fail marker", async () => {
  const { buildTestSkeleton } = await import(join(ROOT, "src/index.js"))
  const skeleton = await buildTestSkeleton("src/app.ts", 'export function app() {}', { strict: true, quality: false })
  assert.ok(skeleton !== null, "skeleton is not null")
...
test("buildTestSkeleton: handles Python files", async () => {
  const { buildTestSkeleton } = await import(join(ROOT, "src/index.js"))
  const skeleton = await buildTestSkeleton("src/main.py", 'def main():\n    pass', { strict: false, quality: false })
...
test("buildTestSkeleton: handles Rust files", async () => {
  const { buildTestSkeleton } = await import(join(ROOT, "src/index.js"))
  const skeleton = await buildTestSkeleton("src/lib.rs", 'pub fn run() {}', { strict: false, quality: false })
...
test("enforceTestFile: skips null for unsupported extension", async () => {
  ...
  const result = await enforceTestFile("src/data.json")
