...
test("session bridge builds a carry-forward prompt for the next session", () => {
  ...
  assert.ok(bridge.verified_difficulty, "bridge should carry a verified local difficulty snapshot")
  assert.equal(bridge.verified_difficulty.level, "simple")
  assert.ok(bridge.route_decision, "bridge should keep the route decision snapshot")
