...
test("message.updated: CLI output shape with content array (Part[])", async () => {
  ...
  assert.ok(FOOTER_BRAND_RE.test(extracted) && extracted.includes("—"), "footer on Content[]: " + extracted.slice(-120))
})
...
test("message.updated: CLI output shape with parts array", async () => {
  ...
  assert.ok(FOOTER_BRAND_RE.test(extracted) && extracted.includes("—"), "footer on Parts[]: " + extracted.slice(-120))
})
...
test("message.updated: nested message payload also receives the footer", async () => {
  ...
  assert.ok(FOOTER_BRAND_RE.test(extracted) && extracted.includes("—"), "nested message payload gets footer: " + extracted.slice(-120))
})
...
test("regression: message.updated empty content does not poison text.complete dedup", async () => {
  ...
  assert.ok(FOOTER_BRAND_RE.test(o.text) && o.text.includes("—"), "footer despite prior empty msg.updated: " + o.text.slice(-120))
})
...
test("regression: footer writes to stderr when stdout not TTY", async () => {
  ...
  try {
    ...
    assert.ok(FOOTER_BRAND_RE.test(all) && all.includes("—"), "footer on stderr: " + all.slice(-120))
  } finally {
