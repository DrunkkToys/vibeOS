...
test("Python: generates test path and imports exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("tests/test_calc.js"))
...
test("TypeScript: generates .test.ts with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("tests/runner.test.js"))
...
test("Go: generates _test.go with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("server_test.js"))
...
test("Rust: generates _test.rs with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("tests/lib_test.js"))
...
test("Ruby: generates _test.rb with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("test/greeter_test.js"))
...
test("Shell: generates test_.sh with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("tests/test_deploy.js"))
...
test("Java: generates Test.java with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("src/test/ServiceTest.js"))
...
test("Kotlin: generates Test.kt with exports", () => {
  ...
  assert.ok(s)
  assert.ok(s.path.endsWith("src/test/CalculatorTest.js"))
...
test("creates skeleton with exports on disk", () => {
  ...
  assert.ok(content.includes("[vibeOS-enforced]"))
...
test("empty file → no exports → placeholder", () => {
  ...
  assert.ok(created, "returned path")
  const content = readFileSync(created, "utf-8")
  assert.ok(content.includes("t.Skip"), "has strict marker")
})
