...
### Fixes
- fix: `vibe uninstall` actually ran nothing — the in-session command resolves `scripts/uninstall.mjs` relative to the *deployed* bundle (`~/.opencode/plugins/vibeOS.js`), where no `scripts/` tree exists, so it always fell through to merely printing the `npx` command. The uninstaller is now bundled to `dist/uninstall.mjs` and deployed next to the plugin, and `trinity-tool` resolves it there first
- fix: an OpenCode process that loaded vibeOS before removal kept the bundle in memory and recreated `$VIBEOS_HOME` and the state files on the next turn — the uninstall marker only gated tier-agent registration. `DelegationEnforcer` now returns zero hooks when the marker is present, making the loaded instance fully inert until reinstall
- fix: uninstall swept only `~/.opencode`; it now sweeps every OpenCode home an install could have targeted (XDG `~/.config/opencode`, the desktop app support dir, the project `.opencode`, and any `VIBEOS_OPENCODE_HOME` override)
- fix: uninstall left `opencode-retention.log` (the launch agent's log sink at the home root, not under `plugins/`), `learned-patterns.json` and `recent-events.jsonl` (the pattern store) behind
- fix: uninstall never touched `opencode.jsonc`, so a vibe registration in the JSONC variant survived
- fix: uninstall left vibeOS auto-generated project skills (`.opencode/skills/<project>/SKILL.md`) behind; they are now removed by their `Auto-generated by vibeOS` marker, and hand-written skills are preserved
- fix: uninstall's plugin-array change detection reported a change when the array was already empty, causing a needless config rewrite
- fix: uninstall threw on an unreadable `HOME` (unguarded `readdirSync`)
- fix: `VIBEOS_UNINSTALL_SKIP_SYSTEM=1` skips the machine-global side effects (crontab, `launchctl bootout`, `npm unlink -g`) which ignore a redirected `HOME` — without it a sandboxed run edited the real user's crontab

### Tests
- test: `tests/test_uninstall_completeness.test.mjs` (8) and `tests/test_uninstalled_plugin_inert.test.mjs` (2) — the uninstaller had no test coverage at all
